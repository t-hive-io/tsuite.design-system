...
async function main() {
  ...
  pages.forEach((page) => {
    ...
    const totalVariants = pageComponents.reduce(
    ...
    console.log(`\n📄 ${page}`);
    console.log(
      `   Components: ${pageComponents.length} | Total Variants: ${totalVariants}`
    );
    console.log('-'.repeat(150));
    console.log(
      '   ' +
    ...
    pageComponents.forEach((cs) => {
      const name =
        cs.name.length > 43 ? cs.name.substring(0, 40) + '...' : cs.name;
      const size = `${cs.width}×${cs.height}`;
      const props = cs.properties.join(', ');
      const propsDisplay =
        props.length > 38 ? props.substring(0, 35) + '...' : props;
