...
async function main() {
  try {
    ...
    const individualComponents = components.filter(
      (c) => c.type === 'COMPONENT'
    );
    ...
    if (componentSets.length > 0) {
      ...
      componentSets.forEach((cs) => {
        const name =
          cs.name.length > 38 ? cs.name.substring(0, 35) + '...' : cs.name;
        const size = `${cs.width}×${cs.height}`;
        const pageName =
          cs.page.length > 23 ? cs.page.substring(0, 20) + '...' : cs.page;
        const props = cs.properties.length > 0 ? cs.properties.join(', ') : '';
        const propsDisplay =
          props.length > 33 ? props.substring(0, 30) + '...' : props;
    ...
    individualComponents.slice(0, 100).forEach((comp) => {
      const name =
        comp.name.length > 38 ? comp.name.substring(0, 35) + '...' : comp.name;
      const size = `${comp.width}×${comp.height}`;
      const pageName =
        comp.page.length > 23 ? comp.page.substring(0, 20) + '...' : comp.page;

      // Format variant properties
      const variantPropsArr = Object.entries(comp.variantProperties).map(
        ([k, v]) => `${k}=${v}`
      );
      const variantPropsStr = variantPropsArr.join(', ');
      const variantDisplay =
        variantPropsStr.length > 38
          ? variantPropsStr.substring(0, 35) + '...'
          : variantPropsStr;
    ...
    if (individualComponents.length > 100) {
      console.log(
        `\n... and ${
          individualComponents.length - 100
