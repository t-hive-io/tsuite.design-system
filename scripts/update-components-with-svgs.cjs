...
// Load metadata
const metadata = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/assets/icons/metadata.json'), 'utf8'));

// Load figma docs to get component to Figma name mapping
const figmaDocs = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/figma-docs.json'), 'utf8'));
...
console.log(`Found ${Object.keys(figmaNameToSvgs).length} unique Figma components`);
...
componentDirs.forEach(componentName => {
  const componentPath = path.join(componentsDir, componentName, `${componentName}.tsx`);
  ...
  if (!svgFiles || svgFiles.length === 0) {
    console.log(`WARNING: ${componentName} (${figmaComponentName}): No SVG files found`);
    skippedCount++;
    return;
  }
  
  console.log(`OK: ${componentName} (${figmaComponentName}): Found ${svgFiles.length} SVG variant(s)`);
  updatedCount++;
