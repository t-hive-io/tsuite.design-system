...
function exploreNode(node, depth = 0, maxDepth = 3, showId = false) {
  ...
  if (node.children && depth < maxDepth) {
    ...
    node.children.forEach((child) =>
      exploreNode(child, depth + 1, maxDepth, showId)
    );
...
async function main() {
  ...
  try {
        ...
        if (page.children) {
          ...
          page.children.slice(0, 10).forEach((frame, frameIdx) => {
            const sizeStr = frame.absoluteBoundingBox
              ? ` (${Math.round(frame.absoluteBoundingBox.width)}×${Math.round(
                  frame.absoluteBoundingBox.height
                )}px)`
              : '';
            console.log(
              `   ${frameIdx + 1}. ${frame.name} [${frame.id}]${sizeStr}`
            );
    ...
    const totalFrames =
      document.children?.reduce(
        (sum, page) => sum + (page.children?.length || 0),
        0
      ) || 0;
