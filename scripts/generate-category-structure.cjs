...
function generateStoryFile(
  ...
  if (hasProperties) {
    ...
    if (svgFiles.length > 1) {
      const conditionals = properties
        .map(([key, prop]) => {
          const propName = toPropertyName(key);
          if (prop.type === 'VARIANT' && prop.variantOptions) {
            return prop.variantOptions
              .map((option) => {
                const matchingSvg = svgFiles.find((f) =>
                  f
                    .toLowerCase()
                    .includes(option.toLowerCase().replace(/[^a-z0-9]/g, ''))
                );
                if (matchingSvg) {
    ...
    defaultArgs = properties
      .map(([key, prop]) => {
        const propName = toPropertyName(key);
        if (prop.type === 'VARIANT') {
          const defaultValue =
            prop.defaultValue ||
            (prop.variantOptions && prop.variantOptions[0]) ||
            '';
...
export const Default: Story = {};`;
  ...
  if (hasProperties) {
    const destructure = properties
      .map(([key]) => toPropertyName(key))
      .join(', ');
    const defaultValuesObj = properties
      .map(([key, prop]) => {
        const propName = toPropertyName(key);
        if (prop.type === 'VARIANT') {
          const defaultValue =
...
async function generateCategoryStructure(category) {
  ...
  for (const component of components) {
    ...
    if (svgFiles.length === 0) {
      ...
      if (component.variantIds && component.variantIds.length > 0) {
        console.log(
          `    🔍 Tried variant IDs: ${component.variantIds
            .slice(0, 3)
            .map((id) => id.replace(':', '-'))
            .join(', ')}${component.variantIds.length > 3 ? '...' : ''}`
        );
      }
      continue;
    }
    ...
    let safeFileName = component.name
      .replace(/[^a-zA-Z0-9]/g, '')
      .replace(/^(\d)/, '_$1');
    ...
    const storyContent = generateStoryFile(
      category,
      component,
      svgFiles,
      displayName
...
async function main() {
  ...
  if (targetCategories.length > 0) {
    console.log(
      `📋 Processing specific categories: ${targetCategories.join(', ')}\n`
    );
    for (const categoryName of targetCategories) {
      const category = categoryPages.find(
        (c) => c.folder.toLowerCase() === categoryName.toLowerCase()
      );
      if (category) {
