...
const meta = {
  title: '2. Main Pages/Colors',
  parameters: {
    layout: 'padded',
    figma: {
      pageId: '5168:44092',
      frames: 8,
      description: 'Digital Twin color range, palette examples, MacBook mockups',
    },
  },
} satisfies Meta;
...
const ColorGroup = ({ title, colorScale }: { title: string; colorScale: Record<string, string> }) => (
  <div style={{ marginBottom: '2rem' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1rem' }}>
      {Object.entries(colorScale).map(([key, value]) => (
        <ColorSwatch key={key} color={value} name={`${title.toLowerCase()}-${key}`} />
      ))}
    </div>
  </div>
);
...
export const AllColors: Story = {
  render: () => (
    <div style={{ padding: '2rem' }}>
      <h1 style={{ marginBottom: '2rem', fontSize: '2rem', fontWeight: 700 }}>Color System</h1>
