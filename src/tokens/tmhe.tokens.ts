/**
 * TMHE Brand Tokens
 * Auto-generated from Figma Variables
 * DO NOT EDIT MANUALLY - Run 'npm run tokens:transform' to regenerate
 */

export const tmheTokens = {
  colors: {
    'Brand/Brand 800': '#5E000F',
    'Brand/Brand 700': '#8C0017',
    'Brand/Brand 600': '#BB001E',
    'Brand/Brand 500': '#CC4056',
    'Brand/Brand 400': '#DD808F',
    'Brand/Brand 100': '#EEBFC7',
    'Primary/Primary 800': '#2B2D42',
    'Primary/Primary 700': '#383A53',
    'Primary/Primary 600': '#535470',
    'Primary/Primary 500': '#74748F',
    'Primary/Primary 400': '#A2A2BB',
    'Primary/Primary 300': '#CBCBDE',
    'Primary/Primary 200': '#EDEDF5',
    'Neutral/Neutral 800': '#222222',
    'Neutral/Neutral 700': '#808080',
    'Neutral/Neutral 600': '#B3B3B3',
    'Neutral/Neutral 500': '#D9D9D9',
    'Neutral/Neutral 400': '#E6E6E6',
    'Neutral/Neutral 300': '#F7F7F7',
    'Neutral/Neutral 200': '#FCFCFC',
    'Neutral/Neutral 100': '#FFFFFF',
    'Neutral/Neutral 50': '#222222',
  },

  radius: {
    sm: '2px',
    md: '4px',
  },

  font: {
    family: {
      Title: 'Titillium Web',
      Body: 'Titillium Web',
    },
    weight: {
      Regular: 'Regular',
      Normal: 'Normal',
      Medium: 'Medium',
      Semibold: 'Semibold',
    },
    size: {
      xxxs: 10,
      xxs: 12,
      xs: 14,
      sm: 16,
      md: 18,
      lg: 28,
    },
  },
} as const;

export type TmheColorKey = keyof typeof tmheTokens.colors;
export type TmheRadiusKey = keyof typeof tmheTokens.radius;
export type TmheFontFamilyKey = keyof typeof tmheTokens.font.family;
export type TmheFontWeightKey = keyof typeof tmheTokens.font.weight;
export type TmheFontSizeKey = keyof typeof tmheTokens.font.size;
